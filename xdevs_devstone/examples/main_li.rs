...
fn main() {
    const WIDTH: usize = 2; //a lo mejor me toca sacar WIDTH y DEPTH del main y hacerlas globales para que los tests puedan usarlas
    const DEPTH: usize = 5;
    ...
    let modelo_final = ModeloFinal::build(generator, Coup::RestoCoup(modelo_li_4));
...
mod test {
    ...
    fn test_li() {
        ...
        let modelo_final: ModeloFinal<W> =
            ModeloFinal::build(generator, Coup::RestoCoup(modelo_li_4));
        let mut simulator = xdevs::simulator::Simulator::new(modelo_final);
        let config = xdevs::simulator::Config::new(0.0, 10.0, 1.0, None);
        simulator.simulate_vt(&config);
