...
fn main() {
    ...
    //Creación del modelo HI
    let atom_d = CoupAtom::new(); //Modelo atómico que va dentro del acoplado (es el acoplado más interno del modelo HI, es CoupD)
    let coup_atom_d: HI<W> = HI::CoupD(atom_d);
    let modelo_hi: CoupHI<W> = CoupHI::build(
        core::array::from_fn(|_| AtomInputSize2::new()),
        Box::new(coup_atom_d),
    );
    let modelo_hi_2: CoupHI<W> = CoupHI::build(
        core::array::from_fn(|_| AtomInputSize2::new()),
        Box::new(HI::RestoCoup(modelo_hi)),
    );
    let modelo_hi_3: CoupHI<W> = CoupHI::build(
        core::array::from_fn(|_| AtomInputSize2::new()),
        Box::new(HI::RestoCoup(modelo_hi_2)),
    );
    let modelo_hi_4: CoupHI<W> = CoupHI::build(
        core::array::from_fn(|_| AtomInputSize2::new()),
        Box::new(HI::RestoCoup(modelo_hi_3)),
    );
    ...
    let modelo_final: ModeloFinal<W> = ModeloFinal::build(generator, HI::RestoCoup(modelo_hi_4));
...
mod test {
    ...
    fn expected_ic(width: usize, profundidad: usize) -> usize {
        let n_ic_expected = if width > 2 {
            (width - 2) * (profundidad - 1)
        } else {
            0
        };
    ...
    fn test_hi() {
        ...
        //Creación del modelo final (modelo HI + atómico generador que mete datos en el puerto del HI)
        let modelo_final: ModeloFinal<W> = ModeloFinal::build(generator, HI::RestoCoup(modelo_hi));
        let mut simulator = xdevs::simulator::Simulator::new(modelo_final);
        let config = xdevs::simulator::Config::new(0.0, 10.0, 1.0, None);
        simulator.simulate_vt(&config);
        ...
        // assert_eq!(expected_ic(WIDTH, DEPTH), modelo_final.get_n_ic());
