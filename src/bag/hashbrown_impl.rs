//! Implementations of the `Bag` trait for various types of the `hashbrown` crate.
...
mod tests {
    ...
    fn hashmap_bag_get_values_yields_all() {
        ...
        bag.add_value((1, 10)).unwrap();
        bag.add_value((2, 20)).unwrap();
        let mut collected: Vec<(u8, u32)> = bag.get_values().collect();
        // HashMap order is unspecified, so sort by key before comparing.
        collected.sort();
        assert_eq!(collected.as_slice(), &[(1, 10), (2, 20)]);
    }
