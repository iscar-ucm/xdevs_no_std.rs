use xdevs::traits::{AbstractSimulator, Component, PartialCoupled};
...
pub mod mod_coup {
    ...
    pub struct ModCoupInput {
        pub mod_coup_input: xdevs::port::Port<bool, 1>,
    }
    impl ModCoupInput {
        #[inline]
        pub const fn new() -> Self {
            Self {
                mod_coup_input: xdevs::port::Port::new(),
            }
        }
    }
    unsafe impl xdevs::traits::Bag for ModCoupInput {
        ...
        fn is_empty(&self) -> bool {
            true && self.mod_coup_input.is_empty()
        }
        #[inline]
        fn clear(&mut self) {
            self.mod_coup_input.clear();
        }
    }
    #[derive(Debug, Default)]
    pub struct ModCoupOutput {
        pub mod_coup_output: xdevs::port::Port<usize, 1>,
    }
    impl ModCoupOutput {
        #[inline]
        pub const fn new() -> Self {
            Self {
                mod_coup_output: xdevs::port::Port::new(),
            }
        }
    }
    unsafe impl xdevs::traits::Bag for ModCoupOutput {
        #[inline]
        fn is_empty(&self) -> bool {
            true && self.mod_coup_output.is_empty()
        }
        #[inline]
        fn clear(&mut self) {
            self.mod_coup_output.clear();
        }
    }
    pub struct ModCoupComponents<const W: usize, const P: usize> {
        comp_atomic: [Atom; W],
        comp_coupled: Box<Coup<W, P>>,
    }
    impl<const W: usize, const P: usize> ModCoupComponents<W, P> {
        #[inline]
        pub fn new(comp_atomic: [Atom; W], comp_coupled: Box<Coup<W, P>>) -> Self {
            Self {
                comp_atomic,
                comp_coupled,
            }
    ...
    pub struct ModCoupComponentsInput<'__xdevs_inner, const W: usize, const P: usize> {
        ...
        pub comp_coupled: <Box<Coup<W, P>> as xdevs::traits::Component>::InputRef<'__xdevs_inner>,
    }
    ...
    pub struct ModCoupComponentsOutput<'__xdevs_inner, const W: usize, const P: usize> {
        ...
        pub comp_coupled: <Box<Coup<W, P>> as xdevs::traits::Component>::OutputRef<'__xdevs_inner>,
    }
    pub struct ModCoup<const W: usize, const P: usize> {
        pub input: ModCoupInput,
        pub output: ModCoupOutput,
        pub t_last: f64,
        pub t_next: f64,
        pub components: ModCoupComponents<W, P>,
    }
    impl<const W: usize, const P: usize> ModCoup<W, P> {
        #[inline]
        pub fn build(comp_atomic: [Atom; W], comp_coupled: Box<Coup<W, P>>) -> Self {
            Self {
                input: ModCoupInput::new(),
                output: ModCoupOutput::new(),
                t_last: 0.0,
                t_next: f64::INFINITY,
                components: ModCoupComponents::new(comp_atomic, comp_coupled),
            }
        }
    }
    unsafe impl<const W: usize, const P: usize> xdevs::traits::Component for ModCoup<W, P> {
        ...
        type InputRef<'__xdevs_ports>
            = &'__xdevs_ports mut ModCoupInput
        where
            Self: '__xdevs_ports;
        type OutputRef<'__xdevs_ports>
            = &'__xdevs_ports ModCoupOutput
        where
            Self: '__xdevs_ports;
        #[inline]
    ...
    unsafe impl<const W: usize, const P: usize> xdevs::traits::PartialCoupled for ModCoup<W, P> {
        type ComponentsInput<'__xdevs_inner>
            = ModCoupComponentsInput<'__xdevs_inner, W, P>
        where
            Self: '__xdevs_inner;
        type ComponentsOutput<'__xdevs_inner>
            = ModCoupComponentsOutput<'__xdevs_inner, W, P>
        where
            Self: '__xdevs_inner;
    }
    unsafe impl<const W: usize, const P: usize> xdevs::traits::AbstractSimulator for ModCoup<W, P> {
        ...
        fn lambda(&mut self, t: f64) {
            if t >= xdevs::traits::Component::get_t_next(self) {
                ...
                let component_outputs: ModCoupComponentsOutput<'_, W, P> =
                    ModCoupComponentsOutput {
        ...
        fn delta(&mut self, t: f64) -> f64 {
            {
                ...
                let mut component_inputs: ModCoupComponentsInput<'_, W, P> =
                    ModCoupComponentsInput {
    ...
    // pub struct ModCoup<const W: usize, const P: usize> {
    //     #[input]
    //     mod_coup_input: xdevs::port::Port<bool, 1>,
    //     #[output]
    //     mod_coup_output: xdevs::port::Port<usize, 1>,
    //     #[components]
...
unsafe impl<const W: usize, const P: usize> Component for mod_coup::Coup<W, P> {
    type Input = <mod_coup::ModCoup<W, P> as xdevs::traits::Component>::Input;
    type Output = <mod_coup::ModCoup<W, P> as xdevs::traits::Component>::Output;
    type InputRef<'a>
        = &'a mut Self::Input
    where
        Self: 'a;
    ...
    fn get_ports(&mut self) -> (Self::InputRef<'_>, Self::OutputRef<'_>) {
        match self {
            mod_coup::Coup::CoupD(d) => d.get_ports(),
            mod_coup::Coup::RestoCoup(r) => r.get_ports(),
        }
    }

    fn get_out_ports(&self) -> Self::OutputRef<'_> {
        match self {
            mod_coup::Coup::CoupD(d) => d.get_out_ports(),
            mod_coup::Coup::RestoCoup(r) => r.get_out_ports(),
        }
    }
}

impl<const W: usize, const P: usize> xdevs::Coupled for mod_coup::ModCoup<W, P> {
