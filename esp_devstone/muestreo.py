...
# UPDATED: The unit at the end (?:\s*([a-zA-Zµ]+))? is now optional
...
def to_seconds(value_str, unit):
    ...
    # UPDATED: If no unit is printed (because of .as_micros()), default to microseconds
    if not unit:
        return val * 1e-6
    ...
    return val * 1e-6 # Fallback to microseconds just in case
...
def run_simulation(bin_name):
    ...
    for line in iter(process.stdout.readline, ''):
        ...
        if match:
            ...
            val = match.group(2)
            unit = match.group(3) # Will be None if missing
            ...
            for width in WIDTHS:
                for depth in DEPTHS:
                    ...
                    for i in range(ITERATIONS):
                        ...
                        # 3. Write data to CSV (Applying 6 decimals/sig-figs limits)
